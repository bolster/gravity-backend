...
class GMMDataSource(object):
    ...
    def get_data_offset(self, lat, lon):
        c = getcontext()
        lat_effective = c.divmod(c.abs(lat), Decimal('5.0'))[1].quantize(Decimal('.001'), rounding=ROUND_DOWN)
        lon_effective = c.divmod(c.abs(lon), Decimal('5.0'))[1].quantize(Decimal('.001'), rounding=ROUND_DOWN)
    ...
    def get_gravitational_acceleration_at_point(self, lat, lon):
        ...
        r = requests.get(url, headers={'Range': 'bytes=%d-%d' % (offset, offset+3)})
